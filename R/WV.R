WV <- function(expression, weights, sample.names = colnames(expression)[1],
  ...
  unused.weights <- weights[!(weights[, gene.names] %in%
                                colnames(expression)[2:ncol(expression)]), ]
  filtered.weights <- weights[weights[, gene.names] %in%
                                colnames(expression)[2:ncol(expression)],
  ...
  filtered.expr <- expression %>%
    dplyr::select(c(tidyselect::all_of(sample.names),
                    tidyselect::all_of(filtered.weights[, gene.names])))
  filtered.expr <- filtered.expr[,colSums(is.na(filtered.expr)) == 0]
  filtered.weights <- filtered.weights[filtered.weights[,gene.names] %in% 
                                         colnames(filtered.expr)[2:ncol(filtered.expr)],]

  if (nrow(filtered.expr) > 0) {
    ## prep for matrix multiplication
    # store sample names in row names
    rownames(filtered.expr) <- filtered.expr[, sample.names]
