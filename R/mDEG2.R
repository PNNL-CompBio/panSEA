mDEG2 <- function(data.list, types, group.names = c("Diseased", "Healthy"), group.names2 = group.names,
                 group.samples = list(
                   2:(0.5 * (ncol(data.list[[1]]) + 1)),
                   (0.5 * (ncol(data.list[[1]]) + 1) + 1):ncol(data.list[[1]])
                 ), group.samples2 = group.samples, feature.names = rep("Gene", length(types)), p = 0.05, 
