...
GSEA_custom <- function(input.df, gmt.list,
  ...
  if (length(Drug.Sets.over) < 2) {
    stop("annotations for 2+ sets are required")
  } # BG 2022-06-25
  annotations <- as.data.frame(annotations)
  ...
  if (marker.select) {
    KSMarkerArray <- matrix(data = NA, nrow = nperm,
                            ncol = length(Drug.Sets.All))
    num.Drug.Sets.all <- length(Drug.Sets.All)
    `%dopar%` <- foreach::`%dopar%`
    
    KSMarkerArray <- foreach::foreach(L = seq_len(ncol(marker.perms)),
                                      .combine = "rbind") %dopar% {
...
drugSAI <- function(data, gmt = NULL, drug = "Drug",
  ...
  EA <- GSEA_custom(
    ...
    min.per.set, convert.synonyms, ties, marker.select
  )
  if (ties) {
