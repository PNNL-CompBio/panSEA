DMAI <- function(drug.sensitivity, gmt = NULL, expression, weights,
  ...
  used.weights <- weights[!(weights[,gene.names] %in% WV.result$unused.weights[,gene.names]),]
  ...
  ### optional: weight permutations (for biomarker selection)
  if (marker.select) {
    WVMarkerArray <- matrix(data = NA, nrow = nperm,
                            ncol = length(Drug.Sets.All))
    num.Drug.Sets.all <- length(Drug.Sets.All)
    `%dopar%` <- foreach::`%dopar%`
    
    WVMarkerArray <- foreach::foreach(L = seq_len(nrow(used.weights)),
                                      .combine = "rbind") %dopar% {
                                        temp.KSMarkerArray <- matrix(data = NA, nrow = 1,
                                                                     ncol = num.Drug.Sets.all)
                                        for (i in seq_len(length(Drug.Sets.All))) {
                                          pos_gene_set <- which(temp.input[, Drug.Sets.All[i]] %in% c("X"))
                                          
                                          # calculate enrichment score for each drug set and rank list which excluded each gene
                                          temp.KSMarkerArray[, i] <- GSEA.EnrichmentScore(marker.perms[,L],
                                                                                          pos_gene_set,
    ...
    colnames(KSMarkerArray) <- Drug.Sets.All
  ...
  DMEA.results <- panSEA::drugSEA_ties(
    ...
    descriptions = descriptions, convert.synonyms = FALSE, ties = ties
  )
